...
void PhysicsAddForceAndGravity (const NewtonBody* body, dFloat timestep, int threadIndex)
{
	...
	Entity = (BallEntity*)NewtonBodyGetUserData(body);
	dVector *force_dir = NULL;
	do
	{
		if(Entity != NULL)
			force_dir = (dVector*)Entity->GetForceVector();
		...
		if(force_dir != NULL)
		{
			...
			dFloat mass;
			NewtonBodyGetMass (body, &mass, &Ixx, &Iyy, &Izz);
			dVector force (force_dir->Scale (mass * 9.81f));
			NewtonBodyAddForce (body, &force.m_x);
			delete force_dir;
		}
	}
	while(force_dir != NULL);
...
//mass = 0.0f;
	dVector force (dir.Scale (mass * -9.81f));
	NewtonBodyAddForce (body, &force.m_x);
...
NewtonBody* CreateSimpleSolid (NewtonWorld *world, dFloat mass, const dMatrix& matrix, NewtonCollision* const collision, int materialId, bool generalInertia)
{
	dAssert (collision);

	// add an new entity to the world
	return CreateSimpleBody (world, NULL, mass, matrix, collision, materialId, generalInertia);
}
/*
...
*/
#define D_PLACEMENT_PENETRATION   0.0001f

bool DoBodiesCollide(NewtonWorld* const world, NewtonBody *BodyA, NewtonBody *BodyB)
{
	bool m_isInPenetration = false;

    NewtonCollision* const collisionA = NewtonBodyGetCollision(BodyA);
    NewtonCollision* const collisionB = NewtonBodyGetCollision(BodyB);

    dMatrix poseA;
    NewtonBodyGetMatrix(BodyA, &poseA[0][0]);

    dMatrix poseB;
    NewtonBodyGetMatrix(BodyB,&poseB[0][0]);
	...
	{
//		printf("Collision Intersection exist between %p %d and %p %d\n", BodyA, NewtonBodyGetID(BodyA), BodyB, NewtonBodyGetID(BodyB));
		return true;
		...
		if (contactCount) {
			...
			{
//				printf("Penetration is %f\n", penetrations[i]);
				m_isInPenetration |= (penetrations[i] > (D_PLACEMENT_PENETRATION * 2.0f));
			}
		}
	}
...
// Adapted from OgreNewtonT https://github.com/Stuggy/ogrenewton
NewtonCollision *ParseEntity (NewtonWorld *world, const MeshPtr mesh, const Matrix4& matrix)
{
	...
	for (unsigned short cs = 0; cs < sub; cs++) {
		...
		for (size_t i = 0; i < poly_count; i++)	{
			...
			NewtonTreeCollisionAddFace(tree, 3, &poly_verts[0][0], 12 * sizeof (Real), cs);
			i_offset += 3;
		}
		i_sptr->unlock();
	}
